...
var count = 5; 							// maximum number of feed items to process per feed
var totalCount = 40; 				// total feed items to output
	...
	$(window).load(function(){
		...
		for (var j = 0; j < totalCount; j++) {
			if(j < outputFeedsContainer.length){
				...
				container.innerHTML += '\
					<li>\
						'+ counter +'. <a target="_blank" title="' + entry.title + '" href="' + entry.link + '">' + entry.title + '</a>\
